...
std::string Distribution = "datosByM.dat";
std::ofstream DatosD(Distribution.c_str());

std::string Entropy = "HByM.dat";
...
int main(){
  ...
  for(int jj = 0; jj < S; ++jj ){
    ...
    for(int ii = 0; ii < P; ++ii){
      Intercambio(V, J, disi(gen1), disi(gen2), disr(gen3));
...
void Intercambio (std::vector<double> & V,double J , int i, int j, double r){
  //Regla de intercambio modelo CyC
  double prom = (V[i]+V[j]) / 2;
  double delta = J*(prom - V[i]) + r*V[i];
  if(i!=j){
    V[i] -= delta;
    V[j] += delta;
  }
